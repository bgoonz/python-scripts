...
class Game(tk.Frame):
    ...
    def add_new_tile(self):
        ...
        while self.matrix[row][col] != 0:
            row = random.randint(0, 3)
            col = random.randint(0, 3)
        self.matrix[row][col] = random.choice(
            [2, 4]
        )  # create new tile of 2 or 4 at any random row & col
        ...
        for i in range(4):
            for j in range(4):
                ...
                if cell_value == 0:
                    ...
                    )  # give bgcolor to empty cell's frame
                    self.cells[i][j][
                        "number"
                    ].configure(  # give bgcolor to empty cell's number
                        bg=c.EMPTY_CELL_COLOR, text=""
                    )
                else:
                    self.cells[i][j][
                        "frame"
                    ].configure(  # give cell colour's value to filled cell's frame
